...
async function crearReporte(req, res) {
  try {
    ...
    if (todosLosFuncionarios.length === 0) {
      return res.json({ success: true, data: [], mensaje: "No se encontraron funcionarios para los criterios seleccionados." });
    }
    ...
    // 3. Buscar las horas extras que correspondan a esos funcionarios
    ...
    // 4. Sumar las horas encontradas al mapa
    ...
    // Lógica de borrado y guardado
    const deleteQuery = { fechaInicioReporte: inicio, fechaFinReporte: fin };
    ...
    for (const r of reportesMap.values()) {
      // --- CAMBIO CLAVE: Se añade un filtro para excluir a los que tienen todo en cero ---
      const totalMinutosRegistrados = r.HEDO + r.HENO + r.HEDF + r.HENF + r.HDF + r.HNF + r.RNO;

      // Solo si el funcionario tiene al menos 1 minuto registrado, se incluye en el reporte
    ...
    // Si después de filtrar no queda nadie, se envía un mensaje claro
...
async function exportarReporteExcel(req, res) {
  try {
    ...
    const query = {
        fechaInicioReporte: { $gte: inicio },
        fechaFinReporte: { $lte: fin }
    };
    if (tipoOperario) {
        query.tipoOperario = tipoOperario;
    }
    ...
    reportes.forEach(r => {
      ...
      const dataRow = worksheet.addRow([
        ...
        parseFloat(totalExtrasDEC.toFixed(2)),
      ]);
